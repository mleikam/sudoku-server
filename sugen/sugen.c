...
struct gridline_def {
	...
	const char		*vl_major;
	const char		*vl_minor;
	const char    *vl_linebreak;
	const char 		*f_pad;
	const char 		*f_blank;	
};

const static struct gridline_def machine_def = {
...
const static struct gridline_def utf8_def = {
	.top = { .start = "\xe2\x95\x94", .mid = "\xe2\x95\x90", .end = "\xe2\x95\x97",
		 .major = "\xe2\x95\xa6", .minor = "\xe2\x95\xa4"},
	.major = { .start = "\xe2\x95\xa0", .mid = "\xe2\x95\x90", .end = "\xe2\x95\xa3",
		.major = "\xe2\x95\xac", .minor = "\xe2\x95\xaa" },
	.minor = { .start = "\xe2\x95\x9f", .mid = "\xe2\x94\x80", .end = "\xe2\x95\xa2",
		.major = "\xe2\x95\xab", .minor = "\xe2\x94\xbc" },
  .bottom = { .start = "\xe2\x95\x9a", .mid = "\xe2\x95\x90", .end = "\xe2\x95\x9d",
			      ...
			      const uint8_t *grid)
{
	...
	for (y = 0; y < DIM; y++) {
		...
		for (x = 0; x < DIM; x++) {
			...
			if (v)
				printf("%s%c%s", def->f_pad, alphabet[v], def->f_pad);
			else
				printf("%s%s%s", def->f_pad, def->f_blank, def->f_pad);
		}

		printf("%s%s", def->vl_major,def->vl_linebreak);
...
"    -x                 Use machine-readable characters.\n"
"\n"
...
static int parse_options(int argc, char **argv, struct options *o)
{
	...
	while ((i = getopt_long(argc, argv, "i:t:m:uax", longopts, NULL)) >= 0)
		switch (i) {
		case 'i':
